...
async def send_request(file_id: str):
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        response = await client.post("http://127.0.0.1:8000/process_photo", params={"file_id": file_id})
        print(f"Response for file_id {file_id}: {response.text}")
        print(datetime.datetime.now())

async def main():
    file_ids = [f"AgACAgIAAxkBAAP_ZEU9JGetRWUE7m0UnhS-PcEyWf4AAtvEMRsd-ihK0_dy7pG-FJABAAMCAAN5AAMvBA" for i in range(20)]
